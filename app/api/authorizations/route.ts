...
export async function GET(req: Request) {
  ...
  const result = await payrocRefundRequest<
    PayrocPaginatedResponse<PayrocAuthorization>
  >("GET", `/authorizations?date=${date}&limit=${limit}`, undefined, null);

  if (!result.ok || !result.data) {
    return NextResponse.json(
      {
        error: "Failed to fetch authorizations from SalonTransact",
  ...
  const filtered = result.data.data.filter((a) =>
    a.paymentId ? merchantPaymentIds.has(a.paymentId) : false
  );
