...
export async function GET(req: Request) {
  ...
  const result = await payrocRefundRequest<
    PayrocPaginatedResponse<PayrocBatch>
  >("GET", `/batches?date=${date}&limit=${limit}`, undefined, null);

  if (!result.ok || !result.data) {
    return NextResponse.json(
      {
        error: "Failed to fetch batches from SalonTransact",
  ...
  return NextResponse.json({
    ...
    debug: {
      date,
      limit,
      scope: user.role === "master portal" ? "master" : "merchant",
    },
  });
}
